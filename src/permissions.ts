...
const PERMISSIONS_PATH = path.join(TCODE_DIR, 'permissions.json')
...
export class PermissionManager {
  ...
  async ensurePathAccess(targetPath: string, intent: PathIntent): Promise<void> {
    ...
    if (!this.prompt) {
      throw new Error(
        `Path ${normalizedTarget} is outside cwd ${this.workspaceRoot}. Start tcode in TTY mode to approve it.`,
      )
    }
  ...
  async ensureCommand(
    ...
    if (!this.prompt) {
      throw new Error(
        `Command requires approval: ${signature}. Start tcode in TTY mode to approve it.`,
      )
    }
  ...
  async ensureEdit(targetPath: string, diffPreview: string): Promise<void> {
    ...
    if (!this.prompt) {
      throw new Error(
        `Edit requires approval: ${normalizedTarget}. Start tcode in TTY mode to review it.`,
      )
    }
