...
async function main(): Promise<void> {
  ...
  try {
    ...
    console.log('tcode installer')
    console.log(`配置会写入 ${TCODE_SETTINGS_PATH}`)
    ...
    const launcherScript = [
      '#!/usr/bin/env bash',
      'set -euo pipefail',
      `exec "${path.join(repoRoot, 'bin', 'tcode')}" "$@"`,
      '',
    ].join('\n')
    ...
    console.log('')
    console.log('安装完成。')
    console.log(`配置文件: ${TCODE_SETTINGS_PATH}`)
    ...
    if (!hasPathEntry(targetBinDir)) {
      ...
      console.log('')
      console.log('现在你可以在任意终端输入 `tcode` 启动。')
    }
  } finally {
    rl.close()
  }
