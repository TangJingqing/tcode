...
const GITIGNORE_COMMENT = '# TCode local artifacts'
const GITIGNORE_ENTRIES = ['.tcode/settings.local.json', '.tcode/sessions/']
...
export function renderInitMiniMd(cwd: string): string {
  ...
  sections.push([
    '# MINI.md',
    '',
    'This file provides guidance to TCode when working with code in this repository.',
    '',
  ])
  ...
  sections.push([
    ...
    '- Keep shared defaults in `~/.tcode/settings.json`; reserve `.tcode/settings.local.json` for project-local overrides.',
...
export async function initializeRepo(cwd: string): Promise<InitReport> {
  ...
  const tcodeDir = path.join(cwd, '.tcode')
  artifacts.push({
    name: '.tcode/',
    status: await ensureDir(tcodeDir),
  })
