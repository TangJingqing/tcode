...
- file review uses standard unified diff output (closer to `git diff`)
- approval view supports `Ctrl+O` expand/collapse plus wheel/page scrolling
- `Ctrl+C` exits cleanly even when an approval prompt is open
- finished tool calls auto-collapse into concise summaries to reduce transcript noise
- explicit background shell commands launched through `run_command` are surfaced as lightweight shell tasks instead of remaining stuck as a forever-running tool call
...
- approval input handling avoids deadlocks where Up/Down/Enter can stop responding
- escape-sequence parsing is hardened so malformed terminal input does not stall key handling
- `run_command` accepts single-string invocations like `"git status"` and auto-splits args
- unknown non-shell commands now request approval instead of being rejected immediately
- clarifying questions are structured through `ask_user`, and the turn pauses until the user replies
